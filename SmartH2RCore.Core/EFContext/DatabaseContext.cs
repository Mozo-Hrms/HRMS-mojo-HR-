...
namespace SmartH2RCore.Core.EFContext
{
    public class DatabaseContext : IdentityDbContext<User, Role, int>, IDatabaseContext
    {
        ...
        public DbSet<LocalizationSetting> LocalizationSetting { get; private set; } = default!;

        public DbSet<AssetSetting> AssetSetting { get; private set; } = default!;

        //public DbSet<Notifications> Notifications { get; private set; } = default!;
        //public DbSet<CompanySettings> CompanySettings { get; private set; } = default!;

        public DbSet<ChangePassword> ChangePassword { get; private set; } = default!;

        public DbSet<ThemeSettings> ThemeSettings { get; private set; } = default!; 
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            {
                var entity = builder.Entity<AssetSetting>();
                entity.ToTable(nameof(AssetSetting), SCHEMA_COMPANY);
                entity.HasKey(x => x.Id);
            }

            {
                var entity = builder.Entity<ChangePassword>();
                entity.ToTable(nameof(ChangePassword), SCHEMA_COMPANY);
                entity.HasKey(x => x.Id);
            }

            //{
            //    var entity = builder.Entity<Notifications>();
            //    entity.ToTable(nameof(Notifications), SCHEMA_COMPANY);
            //    entity.HasKey(x => x.Id);
            //}

            {
                var entity = builder.Entity<ThemeSettings>();
                entity.ToTable(nameof(ThemeSettings), SCHEMA_COMPANY);
                entity.HasKey(x => x.Id);
            }

            {
                var entity = builder.Entity<LocalizationSetting>();
                entity.ToTable(nameof(LocalizationSetting), SCHEMA_COMPANY);
                entity.HasKey(x => x.Id);
            }
            ...
            //{
            //    var entity = builder.Entity<CompanySettings>();
            //    entity.HasKey(p => new { p.CompanyId, p.BranchId ,p.Masters });
            //    entity.ToTable(nameof(CompanySettings), SCHEMA_COMPANY);
            //}
            {
