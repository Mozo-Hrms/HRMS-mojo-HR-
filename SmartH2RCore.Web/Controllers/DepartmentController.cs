...
namespace SmartH2RCore.Web.Controllers
{
    public class DepartmentController : BaseController
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetAllDepartment()  //to display data in department dropdownlist
        {
            var DepartmentAll = await _departmentService.GetAll().Select(d => new { d.Id, d.Name }).ToListAsync();
            return new JsonResult(DepartmentAll);
        }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                await _departmentService.DeleteHardAsync(id);
