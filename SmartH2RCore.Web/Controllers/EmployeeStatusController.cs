...
namespace SmartH2RCore.Web.Controllers
{
    public class EmployeeStatusController : BaseController
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetAllEmployeeStatus()  //to display data in department dropdownlist
        {
            var EmployeeSubTypeAll = await _employeeStatusService.GetAll().Select(d => new { d.Id, d.Name }).ToListAsync();
            return new JsonResult(EmployeeSubTypeAll);
        }
        [HttpGet]
