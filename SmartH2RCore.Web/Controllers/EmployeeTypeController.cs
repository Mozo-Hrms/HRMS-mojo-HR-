...
namespace SmartH2RCore.Web.Controllers
{
    public class EmployeeTypeController : BaseController
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetAllEmployeeTpye()  //to display data in department dropdownlist
        {
            var DepartmentAll = await _employeeTypeService.GetAll().Select(d => new { d.Id, d.Name }).ToListAsync();
